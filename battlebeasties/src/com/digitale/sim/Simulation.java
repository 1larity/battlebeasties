 ...
 *
 * Simulates the actors local to the player and other local dynamic objects
 * ships, enemies, shots etc
 */
...
public class Simulation {
	...
	public static String currentBeastieName;
	public static String opponentCurrentBeastieName;
	...
	private void populate() {
		...
		if (BattleBeasties3d.DEBUG)
			System.out.println("create ship at"
					+ BattleBeasties3d.myCharacter.getFirstname() + "sysx "
					+ BattleBeasties3d.myCharacter.getSysx() + "sysy "
					+ BattleBeasties3d.myCharacter.getSysy() + "sysz "
		...
		if (Ship.STATUS == 1 && BattleBeasties3d.manager.isLoaded("sound/engine.mp3")) {
	...
	private void whoGoesFirst() {
		...
		float currentBeastieSpeed=0;
		float opponentCurrentBeastieSpeed=0;
		for (int i=0;i<2;i++){
			if (BattleBeasties3d.actorsList.get(i).getStatus()==1){
			currentBeastieSpeed=BattleBeasties3d.actorsList.get(i).getVelocity();
		}
		}
		// do the same for opponent
		for (int i=3;i<6;i++){
			if (BattleBeasties3d.actorsList.get(i).getStatus()==1){
				opponentCurrentBeastieSpeed=BattleBeasties3d.actorsList.get(i).getVelocity();
		}
		}
		if (currentBeastieSpeed>opponentCurrentBeastieSpeed){
			turn=1;
		}else if (currentBeastieSpeed==opponentCurrentBeastieSpeed){
			setFirstTurnRandom();
		}else{
			turn=0;
		}
	...
	private void setFirstTurnRandom() {
	...
	private void populateActors() {
		for(int i=0; i<BattleBeasties3d.battlePositions.size();i++)
		{
		Actor thisActor = new Actor();
		thisActor.set_firstname("beast"+i);
		thisActor.set_position(new Vector3(BattleBeasties3d.battlePositions.get(i)));
		thisActor.setHitpoints(100);
		thisActor.setShipname("salx");
		thisActor.set_uid(i);
		thisActor.setStatus(0);
		if (i<3){
			thisActor.yawangle=-90;
		}else {
			thisActor.yawangle=90;
		}
		if (i%3==0){
			...
			thisActor.setShipname("cynomys");
			thisActor.setStatus(1);
		}else if(i%3==2){
			thisActor.setShipname("squirrel");
		}
	...
	public static void populateDust() {
		dusts.clear();
		for (int dust = 0; dust < BattleBeasties3d.dustsize; dust++) {
	...
	public void update(float delta) {
		...
		for (int i = 0; i < BattleBeasties3d.actorsList.size(); i++) {
			BattleBeasties3d.actorsList.get(i).update(delta);
		}
		if (Ship.STATUS == 1) {
			...
			updateCurrentBeastie();
		}
		ship.update(delta);
		//calculate expbar
		exp = 0 + (BattleBeasties3d.myCharacter.getExp() - 0) * (576 - 0)
				/ (currentLevelSize - 0);
		if (BattleBeasties3d.DEEPDEBUG)
	...
	private void updateCurrentBeastie() {
		// check which beasties are primary attacker
		for (int i=0;i<2;i++){
			if (BattleBeasties3d.actorsList.get(i).getStatus()==1){
			currentBeastieName=BattleBeasties3d.actorsList.get(i).get_firstname();
		}
		}
		// do the same for opponent
		for (int i=3;i<6;i++){
			if (BattleBeasties3d.actorsList.get(i).getStatus()==1){
			opponentCurrentBeastieName=BattleBeasties3d.actorsList.get(i).get_firstname();
		}
		}
	}
	...
	public static void updateReticules() {
		...
		for (int i = 0; i < BattleBeasties3d.actorsList.size(); i++) {
			...
			if (actor != null && !(actor.shipname.equals("dead"))
				...
				actor.distanceString = String.format("%.0f",
						actor.distance / 10);
	...
	private void updateStatusline() {
		...
		Renderer.status = "x:" + xprint + " y:" + yprint + " z:" + zprint
				...
				+ BattleBeasties3d.mserverTime + " Ping:"
				+ String.format("%05d", BattleBeasties3d.mnetworkPing) + " FPS:"
		...
		if (shipShot != null) {
			try {
				for (int j = 0; j < BattleBeasties3d.actorsList.size(); j++) {
					Actor actor = BattleBeasties3d.actorsList.get(j); // for every
																// invader
					// for all the shots
					...
					for (int i = 0; i < shots.size(); i++) {
						...
						if (actor.get_uid()<0 
							...
							if (actor.position.dst(currentShot.position) < Invader.INVADER_RADIUS) {
								...
								BattleBeasties3d.MyDataOp.newRequest("hitOpponent#"
										+ actor.get_uid() + "#" + 30);

								if (actor.getHitpoints() > 0) {
									actor.setHitpoints(actor.getHitpoints() - 30);
									BattleBeasties3d.actorsList.set(j, actor);
			...
			try {
				for (int j = 0; j < BattleBeasties3d.actorsList.size(); j++) {
					...
					for (int i = 0; i < missiles.size(); i++) {
						...
						// if actor is an NPC
						if (actor.get_uid()<0 	&& (!actor.getShipname().equals("dead"))) {
							// if shipshot is within enemy radius
							if (actor.position.dst(currentShot.position) < Invader.INVADER_RADIUS) {
								...
								if (actor.getHitpoints() <= 0
									...
									explosions
											.add(new Explosion(actor.position,
													Explosion.SWANSONG));
									BattleBeasties3d.actorsList.remove(j);
								}
	...
	public void repair() {
		if (ship.repairCD == 0) {
			...
			if (Ship.SHIP_HEALTH < Ship.SHIP_MAXHEALTH) {
				for (int i = 0; i < BattleBeasties3d.myInventory.size(); i++) {
					System.out.println("repair clicked ");
					Inventory currentItem = BattleBeasties3d.myInventory.get(i);
				...
				if (usableItem == null) {
					...
					BattleBeasties3d.Error = BattleBeasties3d.REPAIR_ERROR;
				} else {// there is some goop, repair ship
					// get latest from db
					BattleBeasties3d.MyDataOp.get3dChar(Integer
							.valueOf(BattleBeasties3d.currencharacteruid));
					...
					BattleBeasties3d.myCharacter.setHitpoints((int) Ship.SHIP_HEALTH);
				...
				Renderer.toastShown = false;
				BattleBeasties3d.Error = BattleBeasties3d.FULLHP_ERROR;
				}
			}
		}
	}
	...
	public void missile() {
		if (ship.missileCD == 0) {
			...
			for (int i = 0; i < BattleBeasties3d.myInventory.size(); i++) {
				...
				if (currentItem.getSlot_id() == 0) {
					...
					if (currentItem.getSubcat().equals("missile")) {
						...
						// repairpack is the biggest found so far
						if (usableItem == null) {
							usableItem = currentItem;
						} else if (currentItem != null) {
			...
			if (usableItem == null) {
				BattleBeasties3d.Error = BattleBeasties3d.MISSILE_ERROR;
				Renderer.toastShown = false;
			} else {// there is a missile, try launch it
				// check for target lock
				boolean lockFound = false;
				for (int index = 0; index < BattleBeasties3d.actorsList.size(); index++) {
					com.digitale.connex.Actor target = BattleBeasties3d.actorsList
							.get(index);
					if (target.locked) {
						lockFound = true;
						// damage target.
						...
						// reset missile timer
						if (Ship.position.dst(target.position) < 2000) {
							ship.missileCD = 50;
							if (listener != null)
								listener.missile();
							...
							BattleBeasties3d.Error = BattleBeasties3d.RANGE_ERROR;
							Renderer.toastShown = false;
						}
					}
				...
				if (!lockFound) {
					BattleBeasties3d.Error = BattleBeasties3d.MISSILE_LOCK_ERROR;
					Renderer.toastShown = false;
				}
	...
	public void trail() {
		...
		String shipname = BattleBeasties3d.myCharacter.getShipname();
				...
				if (currentShipdef.equals(shipname)) {
					...
					if (BattleBeasties3d.shipdefs.get(i).getJet1x() != 99999) {
						...
						shipTrail = new Trail(
								new Vector3(
										Ship.position.x
												+ BattleBeasties3d.shipdefs.get(i)
														.getJet1x(),
										Ship.position.y
												+ BattleBeasties3d.shipdefs.get(i)
														.getJet1y(),
	...
	public void updateDock() {
		...
		if (docktick >= 60) {
			...
			docktick = 0;
			// update db
			BattleBeasties3d.MyDataOp.dockship(BattleBeasties3d.myCharacter.getUid());
		}
	...
	public void updateUndock() {
		...
		if (docktick >= 40) {
			...
			enginePitch = 0.5f + (Ship.SHIP_VELOCITY - 0) * (2.0f - 0.5f)
					/ (Ship.SHIP_MAXVELOCITY - 0);
			SoundManager.playEngine(enginePitch);
			BattleBeasties3d.MyDataOp.undockship(BattleBeasties3d.myCharacter.getUid());
			ship.update(0);
			/*BattleBeasties3d.MyDataOp.getLocalPlayers(BattleBeasties3d.myCharacter.getX(),
					BattleBeasties3d.myCharacter.getY(),
					BattleBeasties3d.myCharacter.getZ(), true);*/
			BattleBeasties3d.MyDataOp.updateDBShip();
		}
