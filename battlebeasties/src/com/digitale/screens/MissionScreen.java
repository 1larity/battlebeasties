...
public class MissionScreen extends StardustScreen {
	...
	public MissionScreen(Stage stage) {
		...
		BattleBeasties3d.missionDefs.clear();
		BattleBeasties3d.myMissions.clear();
		BattleBeasties3d.MyDataOp.getMissionDefs();
		...
		// Group.debug = true;
		final Label missionTitleLabel = new Label(BattleBeasties3d.missionDefs.get(0).getTitle(),
		...
		final Label missionTextLabel = new Label(BattleBeasties3d.missionDefs.get(0).getText()+
				"\n"+BattleBeasties3d.missionDefs.get(0).getConditions()+
				"\n"+BattleBeasties3d.missionDefs.get(0).getRewards(),
		...
		buttonClose.setClickListener(new ClickListener() {
			...
			public void click(Actor actor, float x, float y) {
				...
				BattleBeasties3d.stationScreen=110;
		...
		buttonNext.setClickListener(new ClickListener() {
			...
			public void click(Actor actor, float x, float y) {
				...
				if (currentMission<BattleBeasties3d.missionDefs.size()-1){
					currentMission=currentMission+1;
					System.out.println("missionID"+currentMission);
				}
				SoundManager.playuiclick();
		...
		buttonAccept.setClickListener(new ClickListener() {
			...
			/*	for (int i = 0; i < Stardust3d.myMissions.size(); i++) {
				...
				BattleBeasties3d.MyDataOp.acceptMission(BattleBeasties3d.missionDefs.get(currentMission).getUid());
				SoundManager.playuiclick();
							...
							public void optionSelected(int option) {
								...
								if (option == 0) {
									...
									System.out.println("movetowarehouse "
											+ newItem.name );
									BattleBeasties3d.MyDataOp.newRequest("movetowarehouse#"
											+newItem.name);
							...
							public void optionSelected(int option) {
								...
								if (option == 0) {
									...
									System.out.println("to cargo "
											+ newItem.name );
									BattleBeasties3d.MyDataOp.newRequest("movetocargo#"
											+newItem.name);
