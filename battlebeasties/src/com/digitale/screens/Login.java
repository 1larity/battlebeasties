	...
	public Login(String newslines) {
		...
		buttonLogin.setClickListener(new ClickListener() {
			...
			public void click(Actor actor, float x, float y) {
				...
				if(BattleBeasties3d.DEBUG) System.out.println("login button pressed");
				...
				if (checkpass(pw, pw) && checkuser(ac)) {
					...
					String passed = BattleBeasties3d.MyDataOp.login(ac, pw);
					if(BattleBeasties3d.DEBUG) System.out.println("passed "+passed);
					if (passed.equals("true")) {
						SoundManager.playconfirm();
						stage.addActor(Actors.bottomToast("Login sucessful",
    							4, skin));
					
						BattleBeasties3d.mloginOK=true;
						BattleBeasties3d.gameMode=3;
						//get players characters
						BattleBeasties3d.populateCharacterlist(BattleBeasties3d.muser);
		...
		buttonOptions.setClickListener(new ClickListener() {
			...
			public void click(Actor actor, float x, float y) {
				if(BattleBeasties3d.DEBUG) System.out.println("options button pressed");
				SoundManager.playuiclick();
				BattleBeasties3d.gameMode=6;
				doneflag=true;
			}
		});
		buttonNewAccount.setClickListener(new ClickListener() {
			...
			public void click(Actor actor, float x, float y) {
				if(BattleBeasties3d.DEBUG) System.out.println("New account button pressed");
				SoundManager.playuiclick();
				BattleBeasties3d.gameMode=14;
				doneflag=true;
			}
		});
		buttonHelp.setClickListener(new ClickListener() {
			...
			public void click(Actor actor, float x, float y) {
				if(BattleBeasties3d.DEBUG) System.out.println("help button pressed");
				SoundManager.playuiclick();
				BattleBeasties3d.gameMode=9;
				doneflag=true;
			}
		});
		checkBox.setClickListener(new ClickListener() {
			...
			public void click(Actor actor, float x, float y) {
				...
				if(BattleBeasties3d.DEBUG) System.out.println("remeber account name pressed");
				BattleBeasties3d.mremeberAccountname = checkBox.isChecked();
				//if user wants to save login
				if(BattleBeasties3d.mremeberAccountname){
					String pw = passwordTextField.getText().toString();
					...
					prefs.putBoolean("nopoly", BattleBeasties3d.mremeberAccountname);
		...
		timer.scheduleAtFixedRate(new TimerTask() {
			public void run() { 
				...
				if (fadeIn){
						...
						if(BattleBeasties3d.DEBUG) System.out.println("fade "+fadeTimer);
					}
