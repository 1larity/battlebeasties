...
public class Vendor extends StardustScreen {
	...
	public Vendor(Stage stage) {
		...
		BattleBeasties3d.vendorInventory.clear();
		BattleBeasties3d.MyDataOp.getVendorInventory(Integer.valueOf("-450"));
		...
		buttonClose.setClickListener(new ClickListener() {
			...
			public void click(Actor actor, float x, float y) {
				...
				BattleBeasties3d.MyDataOp.getInventory(Integer
						.valueOf(BattleBeasties3d.currencharacteruid),false);	
				BattleBeasties3d.stationScreen = 101;
								...
								if (option == 0) {
									if (item.getBind() != 2) {
										...
										System.out.println("Sold "
												+ newItem.name);
										BattleBeasties3d.MyDataOp.newRequest("sale#"
												+ newItem.name);
	...
	private Table initGridWarehouse() {
		...
		for (int x = 0; x < BattleBeasties3d.vendorInventory.size(); x++) {
			System.out.println("init vendor id " + x);
			final Inventory item = BattleBeasties3d.vendorInventory.get(x);
			itemIcon = "data/" + item.getIcon() + ".png";
						...
						DialogListener dialogListener = (new DialogListener() {
							...
							public void optionSelected(int option) {
								...
								if (option == 0) {
									...
									if(item.getValue()<BattleBeasties3d.myCharacter.getCredits()){
									SoundManager.playCoins();
									System.out.println("purchased "
											+ newItem.name);
									BattleBeasties3d.MyDataOp.newRequest("purchase#"
											+ newItem.name);
