...
public class GameLoop extends StardustScreen implements SimulationListener {
	...
	public void update(float delta) {
		...
		if (Ship.STATUS == 1 && BattleBeasties3d.stationScreen == 0) {
		...
		if (Ship.STATUS == 0 && BattleBeasties3d.stationScreen == 0) {
			rectifyTouchCoords(input);
			if (input.isTouched()) {
...
private void checkForSelection(){
	...
	for (int index=0; index < BattleBeasties3d.actorsList.size();index++){
		...
		com.digitale.connex.Actor actor = BattleBeasties3d.actorsList.get(index);
		actor.locked=false;
	...
	public void accellerometer(Input input) {
		// accelerometer
		if (input.getAccelerometerY() < -BattleBeasties3d.deadzoneX || input.getAccelerometerY() > BattleBeasties3d.deadzoneX)
			Ship.yawAngle -= (input.getAccelerometerY() * BattleBeasties3d.accelXsensitivity);

		if (input.getAccelerometerX() < 4-BattleBeasties3d.deadzoneY) {
			Ship.pitchAngle -= Math.abs(input.getAccelerometerX() - (4-BattleBeasties3d.deadzoneY)) * BattleBeasties3d.accelYsensitivity;
		} else if (input.getAccelerometerX() > 5+BattleBeasties3d.deadzoneY) {
			Ship.pitchAngle += Math.abs(input.getAccelerometerX() - ( 5+BattleBeasties3d.deadzoneY)) * BattleBeasties3d.accelYsensitivity;
		}
	}
	...
	private void rightmenu() {
		if ((touch_x > 800 - 64) && (touch_x < 800)) {
			...
			if ((touch_y > 0) && (touch_y < 64)) {
				...
				BattleBeasties3d.Error = BattleBeasties3d.WIP_ERROR;
	...
	private void dockedrightmenu() {
		if ((touch_x > 800 - 64) && (touch_x < 800)) {
			...
			if ((touch_y > 0) && (touch_y < 64)) {
				...
				BattleBeasties3d.stationScreen = 12;
				if (BattleBeasties3d.DEBUG)
				...
				SoundManager.playuiclick();
				BattleBeasties3d.stationScreen = 10;
				if (BattleBeasties3d.DEBUG)
				...
				SoundManager.playuiclick();
				BattleBeasties3d.stationScreen = 1;
			} else if ((touch_y > 480 - 64) && (touch_y < 480)
					&& Renderer.toastShown == false) {
				SoundManager.playuiclick();
				BattleBeasties3d.stationScreen = 11;
				if (BattleBeasties3d.DEBUG)
			...
			for (int i = 0; i < simulation.numMenuItems; i++) {
				...
				if ((int) touch_x > i * 64 && (int) touch_x < (i * 64) + 64) {
					...
					case 0: {
						...
						BattleBeasties3d.stationScreen = 4;
						break;
					}
					case 1: {
						// inventory
						...
						if (Ship.STATUS == 0) {
							BattleBeasties3d.stationScreen = 2;
						} else {
							// we are in space
							// display cargo only
							BattleBeasties3d.stationScreen = 3;
						}
						break;
					}
					case 2: {
						// ship
						BattleBeasties3d.stationScreen = 6;
						simulation.pullout = false;
						break;
					}
					case 3: {
						// char
						BattleBeasties3d.stationScreen = 5;
						simulation.pullout = false;
						break;
					}
					case 4: {
						// market
						BattleBeasties3d.stationScreen = 9;
						simulation.pullout = false;
					...
					case 5: {
						// help
						BattleBeasties3d.stationScreen = 8;
						simulation.pullout = false;
						break;
					}
					case 6: {
						// options
						simulation.pullout = false;
						BattleBeasties3d.stationScreen = 7;
						break;
					}
					case 7: {
						// exit
						...
						DialogListener dialogListener = (new DialogListener() {
							...
							public void optionSelected(int option) {
								...
								// if exit
								if (option == 0) {
									String passed = BattleBeasties3d.MyDataOp
											.postChat(
													BattleBeasties3d.myCharacter
															.getFirstname()
															+ " "
															+ BattleBeasties3d.myCharacter
																	.getSurname()
															+ " has logged out.",
													1, 48);
									passed = BattleBeasties3d.MyDataOp
											.logout(BattleBeasties3d.myCharacter
													.getUid());
									Gdx.app.exit();
								}
							}
					...
					case 8: {
						// options
						simulation.pullout = false;
						BattleBeasties3d.stationScreen = 13;
						break;
					}
