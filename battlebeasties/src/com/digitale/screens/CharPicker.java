...
public class CharPicker extends StardustScreen {
	...
	private String dialogTexts;
       ...
       public CharPicker() {
    	   ...
    	   BattleBeasties3d.currencharacteruid=""+BattleBeasties3d.charList[0].getUid();
    	   int charListLength=0;
    	   for (int i = 0; i < BattleBeasties3d.charList.length; i++) {
    		   
    		   if (BattleBeasties3d.charList[i].getFirstname()!=null){
    			   tempEntries[i]=new String(""); 
    		   tempEntries[i]=(BattleBeasties3d.charList[i].getFirstname() + " "
    					+ BattleBeasties3d.charList[i].getSurname());
    		   tempEntries[i]= tempEntries[i]+("  System " + BattleBeasties3d.charList[i].getSystem()// + " Sta "
    					...
    					+ ".  Flying: "
    					+ Util.asCapFirstChar(BattleBeasties3d.charList[i].getShipname()));
    		   charListLength++;
   		}else{
    	   tempEntries[i]="";
       }
    	   ...
    	   currentAvatar=BattleBeasties3d.charList[0].getAvatar();
    	   OrthographicCamera camera;
              ...
              final List list = new List(listEntries, skin.getStyle(ListStyle.class), "list");    
               setAvatarPicture();
              if(BattleBeasties3d.charList[0].getFirstname()==null){
            	  selectedCharacter="    ";
              }else{
               selectedCharacter=(BattleBeasties3d.charList[0].getFirstname() + " "
   					+ BattleBeasties3d.charList[0].getSurname() );
              }
              final Label fpsLabel = new Label(selectedCharacter, skin.getStyle(LabelStyle.class), "label");
               ...
               dropdown.setSelectionListener(new SelectionListener() {               
				...
				public void selected(Actor actor, int index, String value) {
					SoundManager.playuiclick();
					 selectedCharacter=( BattleBeasties3d.charList[index].getFirstname() 
		    					+" "+ BattleBeasties3d.charList[index].getSurname() );
					 BattleBeasties3d.currencharacteruid=""+BattleBeasties3d.charList[index].getUid();
					 currentAvatar=BattleBeasties3d.charList[index].getAvatar();
					 setAvatarPicture();
           	...
           	buttonEnterGame.setClickListener(new ClickListener() {
    			...
    			public void click(Actor actor, float x, float y) {
    				if(BattleBeasties3d.DEBUG)System.out.println("Charpicker Close");
    				SoundManager.playuiclick();
    				if(BattleBeasties3d.DEBUG)System.out.println("character ID"+BattleBeasties3d.currencharacteruid);
    				if (Integer.valueOf(BattleBeasties3d.currencharacteruid)!=0){
    				//populate game from db, based on this char
    					BattleBeasties3d.gameMode=4;
    					//init character
    					BattleBeasties3d.MyDataOp.get3dChar(Integer.valueOf(BattleBeasties3d.currencharacteruid));
    					//
    					if (!BattleBeasties3d.DEBUG){
    					String passed = BattleBeasties3d.MyDataOp.postChat(BattleBeasties3d.myCharacter.getFirstname()+
    							" "+BattleBeasties3d.myCharacter.getSurname()+" has joined the game.",
    							1, 48);
    					}
    					BattleBeasties3d. MyDataOp.getInventory(Integer.valueOf(BattleBeasties3d.currencharacteruid),true);
    					BattleBeasties3d. MyDataOp.getSolarSystem(BattleBeasties3d.myCharacter.getX(),
    							BattleBeasties3d.myCharacter.getX(),
    							BattleBeasties3d.myCharacter.getZ());
    					...
    					BattleBeasties3d.MyDataOp.start();
           	...
           	buttonNewCharacter.setClickListener(new ClickListener() {
    			...
    			public void click(Actor actor, float x, float y) {
    				...
    				SoundManager.playuiclick();				
    				if (BattleBeasties3d.numberOfCharacters >= 10) {
        	 ...
        	 DialogListener dialogListener=(new DialogListener(){
				...
				public void optionSelected(int option) {
					...
					if (option==0){
						...
						System.out.println("DELETED"+selectedCharacter+" "+BattleBeasties3d.currencharacteruid);
						stage.removeActor(dialog);
					}
	...
	public void setAvatarPicture() {
       ...
       public void render (float delta) {
    		...
    		imageActor.height=128;
               ...
               if(BattleBeasties3d.DEBUG) Table.drawDebug(stage);
       }
