...
public class ShipScreen extends StardustScreen {
	...
	public ShipScreen(Stage stage) {
		...
		BattleBeasties3d.MyDataOp.getShipFitting(BattleBeasties3d.myCharacter.getUid());
		for (int i = 0; i < BattleBeasties3d.charList.length; i++) {
			listEntries[i] = (BattleBeasties3d.charList[i].getFirstname() + " \n"
					+ BattleBeasties3d.charList[i].getSurname() + " \n " + "10000c ");
			listEntries[i] = listEntries[i]
					+ ("System " + BattleBeasties3d.charList[i].getSystem()
							+ " \nSta " + BattleBeasties3d.charList[i].getStamina()
							+ " Int "
							+ BattleBeasties3d.charList[i].getIntelligence()
							+ " Soc " + BattleBeasties3d.charList[i].getSocial()
							+ " Dex " + BattleBeasties3d.charList[i].getDexterity()
							+ " Led " + BattleBeasties3d.charList[i].getLeadership()
							+ " Rec "
							+ BattleBeasties3d.charList[i].getRecuperation()
							+ " \nFlying: " + Util
							.asCapFirstChar(BattleBeasties3d.charList[i]
									.getShipname()));
		}
		
		skin = new Skin(Gdx.files.internal("data/uiskin.json"),
				Gdx.files.internal("data/uiskin.png"));
		sticklebackTexture = BattleBeasties3d.manager.get("data/stickleback.png",Pixmap.class);
		salxTexture = BattleBeasties3d.manager.get("data/salx.png",Pixmap.class);
		cynomysTexture = BattleBeasties3d.manager.get("data/cynomys.png",Pixmap.class);
		squirrelTexture = BattleBeasties3d.manager.get("data/squirrelicon.png",Pixmap.class);
		Texture shipimage;
		
		Gdx.input.setInputProcessor(stage);

		// Group.debug = true;
		final Label shipNameLabel = new Label(BattleBeasties3d.myCharacter.getShipname()
		...
		if (BattleBeasties3d.myCharacter.getShipname().equals("salx")){
			shipimage = new Texture(salxTexture);
		}else if(BattleBeasties3d.myCharacter.getShipname().equals("stickleback")){
			shipimage = new Texture(sticklebackTexture);
		}else if(BattleBeasties3d.myCharacter.getShipname().equals("squirrel")){
		...
		selectedCharacter = ("Character: "
				+ BattleBeasties3d.charList[0].getFirstname() + " \n" + BattleBeasties3d.charList[0]
				.getSurname());

		
		Window window = new Window("Ship Screen",
				skin.getStyle(WindowStyle.class), "shipWindow");
		if(BattleBeasties3d.DEBUG) window.debug();
		window.x = window.y = 0;
		...
		dropdown.setSelectionListener(new SelectionListener() {
			...
			public void selected(Actor actor, int index, String value) {
				...
				selectedCharacter = ("Character: "
						+ BattleBeasties3d.charList[index].getFirstname() + " \n" + BattleBeasties3d.charList[index]
						.getSurname());
				BattleBeasties3d.currencharacteruid = "" + BattleBeasties3d.charList[index].getUid();
		...
		buttonClose.setClickListener(new ClickListener() {
			public void click(Actor actor, float x, float y) {
				...
				SoundManager.playuiclick();
				BattleBeasties3d.stationScreen = 106;
				doneflag = true;
	...
	private Table initGridEquipment() {
		...
		for (int x = 0; x < BattleBeasties3d.myInventory.size(); x++) {
			System.out.println("init equip id " + x +"item id "+ BattleBeasties3d.myInventory.get(x).getInventoryid());
			...
			if (item.getInventoryid() ==BattleBeasties3d.myFitting.offList.get(offSlots)) {
				if (BattleBeasties3d.DEBUG) {
		...
		for (int offSlots=offImageList.size(); offSlots <BattleBeasties3d.myFitting.offList.size(); offSlots++){
		...
		for (int augSlots=augImageList.size(); augSlots <BattleBeasties3d.myFitting.augList.size(); augSlots++){
			final Inventory item = new Inventory();
			 String uidString = "notset";
		...
		for (int defSlots=defImageList.size(); defSlots <BattleBeasties3d.myFitting.defList.size(); defSlots++){
			final Inventory item = new Inventory();
			 String uidString = "notset";
			...
			private void shipItemDialog(final Inventory item,
				...
				DialogListener dialogListener = (new DialogListener() {
					...
					public void optionSelected(int option) {
						...
						if (option == 0) {
							Renderer.stage.removeActor(dialog);
							BattleBeasties3d.myCharacter.setShipid(item.getInventoryid());
							BattleBeasties3d.myCharacter.setShipname(item.getItem());
							BattleBeasties3d.MyDataOp.changeShip(BattleBeasties3d.myCharacter.getUid(),
									item.getInventoryid());
						}
						// if close
						if (option == 1) {
