...
public class DataOpImpl extends Thread implements DataOp {
	...
	public void run() {
		timer.scheduleAtFixedRate(new TimerTask() {
			...
			public void run() {
				...
				if (Ship.STATUS == 1) {
					BattleBeasties3d.refreshActors();
					Simulation.updateReticules();
					Simulation.updateActorFontCache();
					BattleBeasties3d.MyDataOp.updateDBShip();
					get3dChar(Integer.valueOf(BattleBeasties3d.currencharacteruid));
	...
	// server constants
	private static String IP = "localhost"; // laptop IP
	...
	public String getNPCData(String returnString) {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				// RowData = line.split(",");
				BattleBeasties3d.npcList[i].set_race(RowData[0]);
				int mybyte = Integer.valueOf(RowData[1]);
				BattleBeasties3d.npcList[i].set_x((byte) mybyte);

				mybyte = Integer.valueOf(RowData[2]);
				BattleBeasties3d.npcList[i].set_y((byte) mybyte);

				mybyte = Integer.valueOf(RowData[3]);
				BattleBeasties3d.npcList[i].set_z((byte) mybyte);
				if (RowData[4].equals("1")) {
					BattleBeasties3d.npcList[i].set_is_static(true);
				} else {
					BattleBeasties3d.npcList[i].set_is_static(false);
				}
				BattleBeasties3d.npcList[i].set_firstname(RowData[5]);
				BattleBeasties3d.npcList[i].set_surname(RowData[6]);
				// if(Stardust3d.DEBUG) System.out.println(LOG_TAG+
				// "Everworld: line: "+line);
				i++;
			}
	...
	public void getServerTime() {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				BattleBeasties3d.mserverTime = (RowData[0]);
			}
			is.close();
			// if(Stardust3d.DEBUG) System.out.println(LOG_TAG+
			// "Everworld: Result: "+result);
		...
		BattleBeasties3d.mnetworkPing = pingSQLget - pingStart;
		BattleBeasties3d.mParsePing = pingSQLparse - pingSQLget;
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (accountid.length() > 1) {
					...
					BattleBeasties3d.muser = Integer.valueOf(accountid);
				}
			}
			is.close();
		} catch (Exception e) {
		...
		try {
			...
			if (BattleBeasties3d.DEBUG)
				System.out.println(LOG_TAG
						...
						+ e.toString());
			BattleBeasties3d.Error = BattleBeasties3d.NETWORK_DOWN;
		...
		try {
			while ((linebuffer = reader.readLine()) != null) {
				...
				if (linebuffer.length() > 1) {
					...
					BattleBeasties3d.charList[i].setUid(Integer.valueOf(RowData[0]));
					BattleBeasties3d.charList[i].setFirstname(RowData[1]);
					BattleBeasties3d.charList[i].setSurname(RowData[2]);
					BattleBeasties3d.charList[i].setStamina(Integer
							.valueOf(RowData[3]));
					BattleBeasties3d.charList[i].setIntelligence(Integer
							.valueOf(RowData[4]));
					BattleBeasties3d.charList[i].setSocial(Integer
							.valueOf(RowData[5]));
					BattleBeasties3d.charList[i].setDexterity(Integer
							.valueOf(RowData[6]));
					BattleBeasties3d.charList[i].setLeadership(Integer
							.valueOf(RowData[7]));
					BattleBeasties3d.charList[i].setRecuperation(Integer
							.valueOf(RowData[8]));
					BattleBeasties3d.charList[i].setExp(Integer.valueOf(RowData[9]));
					BattleBeasties3d.charList[i].setX(Integer.valueOf(RowData[10]));
					BattleBeasties3d.charList[i].setY(Integer.valueOf(RowData[11]));
					BattleBeasties3d.charList[i].setZ(Integer.valueOf(RowData[11]));
					BattleBeasties3d.charList[i].setSystem(RowData[13]);
					BattleBeasties3d.charList[i].setShipname(RowData[14]);
					BattleBeasties3d.charList[i].setShipicon(RowData[15]);
					BattleBeasties3d.charList[i].setAvatar(RowData[16]);
					if (BattleBeasties3d.DEBUG)
						System.out.println(LOG_TAG + "Everworld: char: "
								+ BattleBeasties3d.charList[i].getFirstname() + " "
								+ BattleBeasties3d.charList[i].getFirstname());
				}
				i++;
			}
	...
	public String getInventory(int characterId, boolean init) {
		...
		try {
			...
			while ((line = reader.readLine()) != null && (line = reader.readLine()).length() > 10) {
				...
				if (thisItem.getItem() != null) {
					BattleBeasties3d.myInventory.add(thisItem);
					i++;
				}
			}
			is.close();
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				if (line.trim().length() > 1) {
					...
					BattleBeasties3d.news[i].set_text(RowData[0]);
					BattleBeasties3d.news[i].set_title(RowData[1]);
					BattleBeasties3d.news[i].set_date(RowData[2]);
					i++;
				}
			}
			is.close();
	...
	public String getFreeUser(String username) {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				String[] RowData = line.split("#");
				if (RowData[0] != null) {
					BattleBeasties3d.muserExists = (RowData[0]);
					i++;
				}
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				if (line.trim().length() > 1) {
					...
					BattleBeasties3d.myCharacter.setUid(Integer.valueOf(RowData[0]));
					BattleBeasties3d.myCharacter.setFirstname(RowData[1]);
					BattleBeasties3d.myCharacter.setSurname(RowData[2]);
					BattleBeasties3d.myCharacter.setStamina(Integer
							.valueOf(RowData[3]));
					BattleBeasties3d.myCharacter.setIntelligence(Integer
							.valueOf(RowData[4]));
					BattleBeasties3d.myCharacter.setSocial(Integer
							.valueOf(RowData[5]));
					BattleBeasties3d.myCharacter.setDexterity(Integer
							.valueOf(RowData[6]));
					BattleBeasties3d.myCharacter.setLeadership(Integer
							.valueOf(RowData[7]));
					BattleBeasties3d.myCharacter.setRecuperation(Integer
							.valueOf(RowData[8]));
					BattleBeasties3d.myCharacter.setExp(Integer.valueOf(RowData[9]));
					BattleBeasties3d.myCharacter.setX(Integer.valueOf(RowData[10]));
					BattleBeasties3d.myCharacter.setY(Integer.valueOf(RowData[11]));
					BattleBeasties3d.myCharacter.setZ(Integer.valueOf(RowData[12]));
					BattleBeasties3d.myCharacter.setSystem(RowData[13]);
					BattleBeasties3d.myCharacter.setShipid(Integer
							.valueOf(RowData[14]));
					BattleBeasties3d.myCharacter.setHitpoints(Integer
							.valueOf(RowData[15]));
					BattleBeasties3d.myCharacter.setShield(Integer
							.valueOf(RowData[16]));
					BattleBeasties3d.myCharacter.setPower(Integer
							.valueOf(RowData[17]));
					BattleBeasties3d.myCharacter.setCpu(Integer.valueOf(RowData[18]));
					BattleBeasties3d.myCharacter.setShipname(RowData[19]);
					BattleBeasties3d.myCharacter.setShipicon(RowData[20]);
					BattleBeasties3d.myCharacter.setSysx(Float.valueOf(RowData[21]));
					BattleBeasties3d.myCharacter.setSysy(Float.valueOf(RowData[22]));
					BattleBeasties3d.myCharacter.setSysz(Float.valueOf(RowData[23]));
					BattleBeasties3d.myCharacter.setStatus(RowData[24]);
					BattleBeasties3d.myCharacter.setCredits(Integer
							.valueOf(RowData[26]));
					i++;
					if (BattleBeasties3d.DEEPDEBUG)
						System.out.println("data ops enter game as"
								+ BattleBeasties3d.myCharacter.getFirstname()
								+ "sysx " + BattleBeasties3d.myCharacter.getSysx()
								+ "sysy " + BattleBeasties3d.myCharacter.getSysy()
								+ "sysz " + BattleBeasties3d.myCharacter.getSysz()
								+ "status "
								+ BattleBeasties3d.myCharacter.getStatus());
				}
			}
			is.close();
		} catch (Exception e) {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				if (line.trim().length() > 1) {
					...
					String[] RowData = line.split("#");
					if (RowData != null) {
						BattleBeasties3d.myFitting.defList.set(0,
								(Integer.valueOf(RowData[0])));
						BattleBeasties3d.myFitting.defList.set(1,
								(Integer.valueOf(RowData[1])));
						BattleBeasties3d.myFitting.defList.set(2,
								(Integer.valueOf(RowData[2])));
						BattleBeasties3d.myFitting.defList.set(3,
								(Integer.valueOf(RowData[3])));
						BattleBeasties3d.myFitting.defList.set(4,
								(Integer.valueOf(RowData[4])));

						BattleBeasties3d.myFitting.offList.set(0,
								(Integer.valueOf(RowData[5])));
						BattleBeasties3d.myFitting.offList.set(1,
								(Integer.valueOf(RowData[6])));
						BattleBeasties3d.myFitting.offList.set(2,
								(Integer.valueOf(RowData[7])));
						BattleBeasties3d.myFitting.offList.set(3,
								(Integer.valueOf(RowData[8])));
						BattleBeasties3d.myFitting.offList.set(4,
								(Integer.valueOf(RowData[9])));

						BattleBeasties3d.myFitting.augList.set(0,
								(Integer.valueOf(RowData[10])));
						BattleBeasties3d.myFitting.augList.set(1,
								(Integer.valueOf(RowData[11])));
						BattleBeasties3d.myFitting.augList.set(2,
								(Integer.valueOf(RowData[12])));
						BattleBeasties3d.myFitting.augList.set(3,
								(Integer.valueOf(RowData[13])));
						BattleBeasties3d.myFitting.augList.set(4,
	...
	public void getSolarSystem(float x, float y, float z) {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				String[] RowData = line.split(",");
				BattleBeasties3d.solarSystem[i].setX(Integer.valueOf(RowData[0]));
				BattleBeasties3d.solarSystem[i].setY(Integer.valueOf(RowData[1]));
				BattleBeasties3d.solarSystem[i].setZ(Integer.valueOf(RowData[2]));
				BattleBeasties3d.solarSystem[i].setSysx(Integer.valueOf(RowData[3]));
				BattleBeasties3d.solarSystem[i].setSysy(Integer.valueOf(RowData[4]));
				BattleBeasties3d.solarSystem[i].setSysz(Integer.valueOf(RowData[5]));
				BattleBeasties3d.solarSystem[i].setModelScale(Integer
						.valueOf(RowData[6]));
				BattleBeasties3d.solarSystem[i].setActivationRadius(Integer
						.valueOf(RowData[7]));
				BattleBeasties3d.solarSystem[i].setName(RowData[8]);
				BattleBeasties3d.solarSystem[i].setStructname(RowData[9]);
	...
	public void getLocalPlayers(int x, int y, int z, boolean init) {
		...
		if (BattleBeasties3d.mserverTime != null) {

			try {
				cal.setTime(sdfin.parse(BattleBeasties3d.mserverTime));
				if (BattleBeasties3d.DEEPDEBUG) {
		...
		try {
			...
			String request = REQUEST_LOCAL_PLAYERS + x + "&y=" + y + "&z=" + z
					+ "&charid=" + BattleBeasties3d.myCharacter.getUid() + "&lu="
					+ currentTime;
			if (BattleBeasties3d.DEEPDEBUG) {
		...
		try {
			...
			// line = reader.readLine();
			if (init)
				BattleBeasties3d.actorsList.clear();
			while ((line = reader.readLine()) != null) {
				if (BattleBeasties3d.DEEPDEBUG) {
				...
				thisActor.set_uid(Integer.valueOf(RowData[6]));
				...
				for (int j = 0; j < BattleBeasties3d.actorsList.size(); j++) {
					String currentActorid = ""
							+ BattleBeasties3d.actorsList.get(j).get_uid();
					...
					if (currentActorid.equals(thisActorid)) {
						if (thisActor.getShipname().equals("dead")
							...
							if ( !BattleBeasties3d.actorsList.get(j).exploding) {
								Simulation.explosions
										.add(new Explosion(thisActor.position,
												Explosion.SWANSONG));
								BattleBeasties3d.actorsList.get(j).exploding = true;
								if (Simulation.listener != null)
									Simulation.listener.boom();
								// Stardust3d.actorsList.remove(j);
							}
							BattleBeasties3d.actorsList.get(j).shipname = thisActor.shipname;
							// Stardust3d.actorsList.remove(j);

							// maybe notify renderer for explosion
						} else {
							// update the existing actor
							BattleBeasties3d.actorsList.get(j).setHitpoints(
									thisActor.getHitpoints());
							BattleBeasties3d.actorsList.get(j).setPitchangle(
									thisActor.getPitchangle());
							BattleBeasties3d.actorsList.get(j).setYawangle(
									thisActor.getYawangle());
							BattleBeasties3d.actorsList.get(j).position = thisActor
									.get_position();
							BattleBeasties3d.actorsList.get(j).shipname = thisActor.shipname;
							BattleBeasties3d.actorsList.get(j).setStatus(
									thisActor.getStatus());
						}
						updated = true;
				...
				// new actor
				if (updated == false) {
					BattleBeasties3d.actorsList.add(thisActor);
					if (BattleBeasties3d.DEEPDEBUG) {
		...
		try {
			...
			while ((linebuffer = reader.readLine()) != null) {
				...
				if (linebuffer.length() > 1) {
					...
					BattleBeasties3d.assetList.add(thisAssetDef);
					i++;
				}
			}
		...
		try {
			...
			while ((linebuffer = reader.readLine()) != null) {
				...
				if (linebuffer.length() > 1) {
					...
					BattleBeasties3d.channelList.add(thisChannelDef);
					i++;
				}
			}
			...
			while ((linebuffer = reader.readLine()) != null) {
				...
				if (linebuffer.length() > 1) {
					...
					if (thisShipDef.getModel() != null) {
						BattleBeasties3d.shipdefs.add(thisShipDef);
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (line.trim().length() > 1) {
					...
					BattleBeasties3d.generatedFirstName = (RowData[0]);
					BattleBeasties3d.generatedSurName = (RowData[1]);

					if (BattleBeasties3d.DEBUG)
						System.out.println("random name "
								+ BattleBeasties3d.generatedFirstName + " "
								+ BattleBeasties3d.generatedSurName);
				}
	...
	public void getMissionDefs() {
		...
		MCrypt mcrypt = new MCrypt();
		String request = REQUEST_ENCRYPTED;
		String data = "24#" + BattleBeasties3d.myCharacter.getUid();
		String encrypted = "";
		...
		try {
			...
			while ((linebuffer = reader.readLine()) != null) {
				...
				if (linebuffer.length() > 1) {
					...
					BattleBeasties3d.missionDefs.add(thisMissionDef);
					i++;
				}
			}
	...
	public void updateDBShip() {
		...
		String data = "12#" + BattleBeasties3d.myCharacter.getSysx() + "#"
				+ BattleBeasties3d.myCharacter.getSysy() + "#"
				+ BattleBeasties3d.myCharacter.getSysz() + "#"
				+ BattleBeasties3d.myCharacter.getUid() + "#"
				+ BattleBeasties3d.myCharacter.getVelocity() + "#"
				+ BattleBeasties3d.myCharacter.getPitchAngle() + "#"
				+ BattleBeasties3d.myCharacter.getYawAngle() + "#"
				+ BattleBeasties3d.myCharacter.getHitpoints();
	...
	public void getchat(int selectedChannel, boolean reset,
		...
		if (!serverMessages) {
			...
			if (BattleBeasties3d.chatList.size() > 0) {
				latestChat = "" + BattleBeasties3d.chatList.get(0).getTimestamp();
			} else {
			...
			// get last chat timestamp
			if (BattleBeasties3d.serverChatList.size() > 0) {
				latestChat = ""
						+ BattleBeasties3d.serverChatList.get(0).getTimestamp();
			} else {
		...
		try {
			...
			while ((linebuffer = reader.readLine()) != null) {
				...
				if (linebuffer.length() > 1) {
					...
					if (reset == false && BattleBeasties3d.chatChannel != 1) {
					...
					if (serverMessages) {
						BattleBeasties3d.serverChatList.add(0, thischatline);
					} else {
						BattleBeasties3d.chatList.add(0, thischatline);
					}
					i++;
				}
			}
		...
		if (clientRequests.size() > 0) {
			...
			if (command.equals("hitOpponent")) {
				...
				sell(BigInteger.valueOf(Long.valueOf(RowData[1])),
						BigInteger.valueOf(Long.valueOf(BattleBeasties3d.myCharacter
				...
				inventoryOp(BigInteger.valueOf(0), 3, BigInteger.valueOf(Long
						.valueOf(BattleBeasties3d.myCharacter.getUid())));
			}
		}
	}
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (BattleBeasties3d.DEBUG)
					System.out.println(i + thisItem.getItem());
				if (thisItem.getItem() != null) {
					BattleBeasties3d.vendorInventory.add(thisItem);
					i++;
				}
			}
			is.close();
		...
		try {
			...
			while ((linebuffer = reader.readLine()) != null) {
				...
				if (linebuffer.length() > 1) {
					...
					BattleBeasties3d.avatarList.add(thisAvatarDef);
					i++;
				}
			}
	...
	public void acceptMission(BigInteger uid) {
		...
		MCrypt mcrypt = new MCrypt();
		String request = REQUEST_ENCRYPTED;
		String data = "25#" + uid + "#" + BattleBeasties3d.myCharacter.getUid();
		if (BattleBeasties3d.DEBUG)
	...
	public void getMissionLog(int completed) {
		...
		MCrypt mcrypt = new MCrypt();
		String request = REQUEST_ENCRYPTED;
		String data = "26#" + BattleBeasties3d.myCharacter.getUid() + "#" + completed;
		String encrypted = "";
		...
		try {
			...
			while ((linebuffer = reader.readLine()) != null) {
				...
				if (linebuffer.length() > 1) {
					...
					BattleBeasties3d.myMissions.add(thisMissionLog);
					i++;
				}
			}
	...
	@Override
	public String logout(int muser) {
		HttpClient httpclient = HttpClientFactory.getThreadSafeClient();
		InputStream is = null;
		String result = "";
		if (BattleBeasties3d.DEBUG)
